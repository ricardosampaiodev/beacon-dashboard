      ...
      <div className={styles['chart-wrapper']}>
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={chartData} margin={{ top: 10, right: 15, left: 15, bottom: 0 }}>
            ...
            <Tooltip 
              contentStyle={{ background: '#0e0f11', border: '1px solid #5f646d', borderRadius: '8px' }}
              itemStyle={{ color: '#e9e9e9' }}
              labelStyle={{ display: 'none' }}
              formatter={(value: any) => [`$${Number(value).toFixed(2)}`, 'Preço']}
            />
            <XAxis dataKey="name" stroke="#7b7b7b" axisLine={false} tickLine={false} tick={{ fontSize: 14, fontFamily: 'Poppins' }} dy={10} interval={0} />
            <Area 
              type="monotone" 
              dataKey="val" 
              stroke="#4c8ff8" 
